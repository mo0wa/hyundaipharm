$(document).ready(function() {
  ...
  if (windowWidth > 1024) {
    if ($("body").find(".Wide").length) {
      ...
      $("#wrap").addClass("Wide");
       /* brand text */
       $(".brand_contents_top > .brand_content_textbox > p").html(
        '현대약품은 "인류보건 향상"의' + "<br>" +
        "기업이념을 바탕으로 모두가 건강하고 따뜻한" + "<br>" +
        "세상을 만들기 위한 지속적인 노력을 해오고 있습니다."
    );
    }
  }
...
$(window).on('scroll', function() {
    ...
    $('.brand_section_wrap > div').each(function(){
    ...
    $('.vision_content_wrap>div').each(function(){
      var contentOffset = $(this).offset().top;
      var contentHeight = $(this).outerHeight();
      if (scrollTop + 500 >= contentOffset) { // 스크롤 위치가 대상 요소의 상단 위치에 도달하면
        $(this).addClass('on'); // 클래스 이름을 추가합니다.
    ...
    $('.products_textbox').each(function(){
      var contentOffset = $(this).offset().top;
      var contentHeight = $(this).outerHeight();
      if (scrollTop + 500 >= contentOffset) { // 스크롤 위치가 대상 요소의 상단 위치에 도달하면
        $(this).addClass('on'); // 클래스 이름을 추가합니다.
    ...
    $('.banner_section_wrap').each(function(){
      var contentOffset = $(this).offset().top;
      var contentHeight = $(this).outerHeight();
      if (scrollTop + 500 >= contentOffset) { // 스크롤 위치가 대상 요소의 상단 위치에 도달하면
        $(this).addClass('on'); // 클래스 이름을 추가합니다.
    ...
    $('.mecenat_section_wrap > div > div').each(function(){
      var contentOffset = $(this).offset().top;
      var contentHeight = $(this).outerHeight();
      if (scrollTop + 700 >= contentOffset) { // 스크롤 위치가 대상 요소의 상단 위치에 도달하면
        $(this).addClass('on'); // 클래스 이름을 추가합니다.
    } else if(scrollTop > contentOffset + contentHeight || scrollTop + windowHeight < contentOffset) {
        $(this).removeClass('on'); // 그렇지 않으면 클래스 이름을 제거합니다.
    }
    });
    $('.news_section_textbox').each(function(){
      var contentOffset = $(this).offset().top;
      var contentHeight = $(this).outerHeight();
      if (scrollTop + 500 >= contentOffset) { // 스크롤 위치가 대상 요소의 상단 위치에 도달하면
        $(this).addClass('on'); // 클래스 이름을 추가합니다.
